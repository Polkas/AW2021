...
class CaBase():
    ...
    def __init__(self, data, seed = 1234):
        assert isinstance(data, pd.DataFrame), "data argument has to be a pandas.DataFrame"
        np.random.seed(seed)
    def apply_svd(self, data):
        ...
        # ipdb.set_trace()
        D_r_rsq =  sparse.diags(1. / np.sqrt(r.A1))
        D_c_rsq =  sparse.diags(1. / np.sqrt(c.A1))
...
class CaPlotMixin():
    def plot(self):
        ...
        ax = fig.gca()
        ax.set(xlim = (np.min(self.data_out.iloc[:, 0]) * 1.1, np.max(self.data_out.iloc[:, 0]) * 1.1),
                ylim = (np.min(self.data_out.iloc[:, 1]) * 1.1, np.max(self.data_out.iloc[:, 1]) * 1.1))
        plt.grid()
        for i, l in enumerate(self.data_out.index):
            ax.text(self.data_out.iloc[i, 0], 
...
class MCA(CaBase, CaScreeMixin, CaPlotMixin):
    """Multiply Correcpondence Analysis.
    Inputs
    ------
    data : pandas.DataFrame 
      Orginal dataset with only binary variables in the long format is expected. 
    ...
    def __init__(self, data: pd.DataFrame) -> None:
        ...
        colors_map = dict(zip(cc_set, (list("bgrcmyk") + list("bgrcmyk") *  (cc_set_len // 7))[:cc_set_len]))
